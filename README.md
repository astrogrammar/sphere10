...
## 🚀 v3.0 アップグレード内容 (2025/12/17)

### 🕰️ 次世代タイムコントロール (Time Cockpit)
- **フローティングウィンドウ**: ドラッグ移動可能な独立パネルを採用。天球の観察を妨げない自由なレイアウトが可能。
- **バックグラウンド再生**: パネルを閉じても時間は止まらない「常時稼働（Always-on）」設計。
- **マルチスケール時間操作**:
  - `Minute` / `Hour`: 出生時間の微調整やハウスの移動確認
  - `Day`: 太陽や内惑星のトラッキング（デフォルト）
  - `Month` / `Year`: 外惑星のトランジットや長期サイクルの観察
- **視認性の向上**: 24pxの大型フォントを採用したデジタル表示カウンター。

### 🏗️ アーキテクチャの刷新 (Logic Separation)
- **ロジック分離**: 肥大化した `sphere10.js` から日時操作ロジックを `scripts/timeControl.js` へ完全分離。
- **Bridge Pattern**: `window.Sphere10` APIを通じた疎結合な連携により、描画エンジンの安定性を確保。
- **Legacy Sync**: 隠し要素（Ghost Input）を用いたイベントブリッジにより、既存の `chart.js`（2Dホロスコープ）との完全な互換性を維持。

### 🎨 UI/UX モダン化
- **Dock System**: 画面右下に機能トグル（[D]日時, [S]恒星, [☿]惑星）を集約したドックランチャーを新設。
- **Glassmorphism**: 半透明のダークテーマとブラー効果（Backdrop Filter）による没入感のあるデザイン。
